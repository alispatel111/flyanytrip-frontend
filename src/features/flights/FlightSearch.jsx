        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2">
          ...
          <div className="relative min-w-[200px]">
            ...
            >
              ...
              <div className="text-sm font-black text-brand-black truncate">{adults} Adult, {travelClass}</div>
            </div>
            <AnimatePresence>
              {showTravelersMenu && (
                <motion.div
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <button onClick={() => setAdults(Math.max(1, adults - 1))} className="w-6 h-6 border rounded-full flex items-center justify-center font-bold">-</button>
                        <span className="font-bold text-sm">{adults}</span>
                        <button onClick={() => setAdults(adults + 1)} className="w-6 h-6 border rounded-full flex items-center justify-center font-bold">+</button>
                      </div>
                    </div>
                    <div className="pt-4 border-t border-black/5">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {['Economy', 'Business'].map(cls => (
                          <button key={cls} className={`py-2 rounded-lg text-xs font-bold transition-all ${travelClass === cls ? 'bg-[#448AFF] text-white' : 'bg-black/5'}`} onClick={() => setTravelClass(cls)}>
                            {cls}
                          </button>
                        ))}
                      </div>
                    </div>
                    <button className="w-full mt-2 bg-brand-black text-white py-2 rounded-lg text-xs font-bold" onClick={() => setShowTravelersMenu(false)}>Apply</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
